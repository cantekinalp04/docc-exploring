...
for ARCHIVE in doc_archives/*.doccarchive; do
    ...
    $(xcrun --find docc) process-archive transform-for-static-hosting "$ARCHIVE" --hosting-base-path docc-exploring/$ARCHIVE_NAME --output-path docs/$ARCHIVE_NAME
done
...
git remote set-url origin https://cantekinalp04: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@github.com/cantekinalp04/docc-exploring/
